...
function selectExtra(extra) {
    // Permitir múltiples extras (doble fijador y feromonas pueden seleccionarse juntos)
...
// Función para solicitar datos del cliente y procesar pedido de producto
...
// ===== INTEGRACIÓN CON SISTEMA DE DATOS DEL CLIENTE =====

// Función mejorada para proceder al pago con datos del cliente
...
// Función para procesar checkout con datos del cliente
function processCheckoutWithCustomerData(customerData) {
    ...
    let mensaje = `🛒 *Pedido Completo - ${carritoItems.length} ${carritoItems.length === 1 ? 'Producto' : 'Productos'}*\n\n`;
    
    // Detalles de productos
    carritoItems.forEach((item, index) => {
        mensaje += `${index + 1}. *${item.producto}*\n`;
        mensaje += `   📦 Tamaño: ${item.tamaño}\n`;
        mensaje += `   💰 Precio base: $${item.precioBase}\n`;
    ...
    if (entregaDomicilio) {
        ...
        mensaje += `🏪 Retiro en tienda: Sin costo adicional\n`;
    }
    ...
    mensaje += `\n\n📝 Por favor confirma disponibilidad, método de pago y tiempo de entrega.`;
...
// Función para mostrar confirmación de checkout
function showCheckoutConfirmation(total, itemCount) {
    ...
    confirmation.innerHTML = `
        <div style="color: #00ff88; font-size: 3rem; margin-bottom: 1rem;">✅</div>
...
// Función mejorada para contactar por WhatsApp con datos del cliente
function contactForCartWithCustomerData() {
    ...
    customerDataManager.requestData('cart').then((customerData) => {
        ...
        let mensaje = `🛒 *Consulta de Pedido - ${carritoItems.length} ${carritoItems.length === 1 ? 'Producto' : 'Productos'}*\n\n`;
        ...
        mensaje += `\n\n📝 Me gustaría consultar disponibilidad y opciones de pago para este pedido.`;
...
console.log('✅ Integración de datos del cliente agregada al sistema de carrito');